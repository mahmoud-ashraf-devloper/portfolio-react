...
const Slider = ({ item }) => {
  ...
  return (
    <div className="relative h-full bg-white md:rounded-r-xl  rounded-xl md:rounded-none overflow-hidden shadow-xl">
      {/* Carousel wrapper */}
      <div className="relative shadow-md h-56  md:h-96">
        ...
        <div className="image-wrap">
          <img
            loading="lazy"
            src={item.baseUrl + item.images[currentImage]}
            className=""
            alt="..."
          />
        </div>
      ...
      <div className="absolute opacity-60 hover:opacity-100 transition duration-200 delay-75 ease-in-out shadow-lg bg-black/60 px-6 py-2 rounded-md z-30 flex space-x-3 -translate-x-1/2 bottom-2 left-1/2 md:overflow-hidden overflow-scroll">
        {item.images.map((image, index) => (
          <button
            ...
            className={
              currentImage === index
                ? " hover:transform hover:-translate-y-2  outline outline-4 outline-indigo-400  transition duration-300 ease-in-out rounded-md"
                : "bg-slate-900/70 hover:transform hover:-translate-y-2 hover:transition hover:duration-300 hover:ease-in-out  rounded-full"
            }
          >
            <img
              className="md:min-w-[40px] md:h-[35px] min-w-[30px] h-[30px] bg-cover shadow-md rounded-md"
              src={item.baseUrl + image}
              alt=""
              srcset=""
            />
          </button>
      ...
      <button
        onClick={() =>
          currentImage - 1 >= 0 ? slideTo(currentImage - 1) : null
        }
        type="button"
      ...
      <button
        onClick={() =>
          currentImage + 1 <= item.images.length - 1
